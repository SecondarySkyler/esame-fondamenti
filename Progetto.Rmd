...
# Vediamo come è cambiato l'andamento della temperature della superficie terrestre
...
#animate(global_mean_temperature.animation, height = 500, width = 800, fps = 30, end_pause = 60, res = 100, duration = 10, renderer = gifski_renderer("test.gif"))

```

# Grazie a questa mappa interattiva è possibile analizzare il cambiamento delle temperature superficiali del nostro pianeta
...
```{r, echo=FALSE, mappa-temperatura}
...
Diamo un'occhiata a come sono cambiate le emissioni di CO2 nel corso degli anni
...
#animate(world_co2_emission.animation, height = 500, width = 800, fps = 30, end_pause = 60, res = 100, duration = 10, renderer = gifski_renderer("world_co2.gif"))
...
co2_map_percapita <- plot_geo(co2_emission_percapita) %>%
  ...
  layout(title = "Emissioni CO2 pro capite, 2019") %>%
  config(displayModeBar = FALSE)
...
Nonostante la Cina sia la nazione che negli ultimi anni ha avuto i livelli di emissione di CO2 più alti, mantiene un livello pro capite relativamente basso, non rientrando nella top 10.
...
Vediamo come i principali stati producono energia
```{r, echo=FALSE, fonti-energetiche}
electricity_prod <- read_csv("datasets/electricity-prod-source-stacked.csv")

electricity_prod %>%
  filter(Entity == "Portugal")

state_filter <- c("France", "Australia")
...
#          value = "TWh produced",
#          c(-Entity, -Code, -Year)) %>%
#   filter(Year == 2019, Entity %in% state_filter, Electricity %in% c("Electricity from coal (TWh)", "Electricity from gas (TWh)")) %>%
#   ggplot(aes(x = Entity, y = `TWh produced`, fill = Electricity)) +
